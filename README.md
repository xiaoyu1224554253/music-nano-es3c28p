...
For the first step, this example performs device discovery to search for a target device (A2DP sink) whose device name is "TWS" and whose "Rendering" bit of its Service Class field is set in its Class of Device (COD). If a candidate target is found, the local device will initiate connection with it.
...
```
I (4090) BT_AV: Found a target device, address xx:xx:xx:xx:xx:xx, name TWS
I (4090) BT_AV: Cancel device discovery ...
I (4100) BT_AV: Device discovery stopped.
I (4100) BT_AV: a2dp connecting to peer: TWS
```
