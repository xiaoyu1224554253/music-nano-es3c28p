...
//                        pPage->JPCI[i].v_samp_factor = ucSamp & 0xf;
                    ...
                    /* 三分量都是 1x2 (色度也做 2:1 垂直抽样) 的 JPEG:
                       0x12 路径假设色度 1x1 (每 MCU 4 块), 而这类图每 MCU 是 6 块,
                       会 Huffman 失步 → 用内部码 0x13 走专用解码分支 */
                    if (pPage->ucNumComponents == 3 &&
                        pPage->ucSubSample == 0x12 &&
                        aSamp[1] == 0x12 && aSamp[2] == 0x12)
                    {
                        pPage->ucSubSample = 0x13;
                    }
                }
...
static int DecodeJPEG(JPEGIMAGE *pJPEG)
{
    ...
    for (y = 0; y < cy && bContinue && iErr == 0; y++)
    {
        ...
        for (x = 0; x < cx && bContinue && iErr == 0; x++)
        {
            ...
            if (pJPEG->ucSubSample == 0x13)
            {
                /* 三分量都 1x2: 每 MCU 6 块 = Y0,Y1,Cb0,Cb1,Cr0,Cr1。
                   色度两块一组(共用 DC 预测), 解出后垂直平均塞进 MCU2/MCU3,
                   复用 JPEGPutMCU12 的 8x16 绘制(色度按 2:1 垂直降采样, 封面尺寸下无感) */
                if (pJPEG->ucPixelType >= EIGHT_BIT_GRAYSCALE)
                {
                    /* 不用色度, 只需按 4 块消费比特流保持同步 */
                    for (i = 0; i < 4; i++)
                    {
                        if (pJPEG->ucMode == 0xc2) { // progressive
                            iErr |= JPEGDecodeMCU_P(pJPEG, MCU_SKIP, (i < 2) ? &iDCPred1 : &iDCPred2);
                        } else {
                            iErr |= JPEGDecodeMCU(pJPEG, MCU_SKIP, (i < 2) ? &iDCPred1 : &iDCPred2);
                        }
                    }
                }
                else
                {
                    /* Cb0, Cb1 -> comp1 表 (注意 MCU2/MCU3 间隔 DCTSIZE, 不能 MCU2+i) */
                    pJPEG->ucACTable = cACTable1;
                    pJPEG->ucDCTable = cDCTable1;
                    for (i = 0; i < 2; i++)
                    {
                        int blk = (i == 0) ? MCU2 : MCU3;
                        if (pJPEG->ucMode == 0xc2) { // progressive
                            iErr |= JPEGDecodeMCU_P(pJPEG, blk | iSkipMask, &iDCPred1);
                        } else {
                            iErr |= JPEGDecodeMCU(pJPEG, blk | iSkipMask, &iDCPred1);
                        }
                        if (pJPEG->u16MCUFlags == 0 || bThumbnail) // no AC components, save some time
                        {
                            c = ucRangeTable[((iDCPred1 * iQuant2) >> 5) & 0x3ff];
                            l = c | ((uint32_t)c << 8) | ((uint32_t)c << 16) | ((uint32_t)c << 24);
                            pl = (uint32_t *)&pJPEG->sMCUs[blk];
                            for (int j = 0; j < iMaxFill; j++) pl[j] = l;
                        }
                        else
                        {
                            JPEGIDCT(pJPEG, blk, pJPEG->JPCI[1].quant_tbl_no);
                        }
                    }
                    /* Cr0, Cr1 -> comp2 表 (MCU4/MCU5) */
                    pJPEG->ucACTable = cACTable2;
                    pJPEG->ucDCTable = cDCTable2;
                    for (i = 0; i < 2; i++)
                    {
                        int blk = (i == 0) ? MCU4 : MCU5;
                        if (pJPEG->ucMode == 0xc2) { // progressive
                            iErr |= JPEGDecodeMCU_P(pJPEG, blk | iSkipMask, &iDCPred2);
                        } else {
                            iErr |= JPEGDecodeMCU(pJPEG, blk | iSkipMask, &iDCPred2);
                        }
                        if (pJPEG->u16MCUFlags == 0 || bThumbnail) // no AC components, save some time
                        {
                        ...
                        {
                            JPEGIDCT(pJPEG, blk, pJPEG->JPCI[2].quant_tbl_no);
                        }
                    }
                    /* 垂直 2:1 平均 -> JPEGPutMCU12 读 MCU2(当Cb) 和 MCU3(当Cr) */
                    uint8_t *mc2 = (uint8_t *)&pJPEG->sMCUs[MCU2];
                    uint8_t *mc3 = (uint8_t *)&pJPEG->sMCUs[MCU3];
                    uint8_t *mc4 = (uint8_t *)&pJPEG->sMCUs[MCU4];
                    uint8_t *mc5 = (uint8_t *)&pJPEG->sMCUs[MCU5];
                    for (i = 0; i < DCTSIZE; i++)
                    {
                        mc2[i] = (uint8_t)((mc2[i] + mc3[i]) >> 1);
                        mc3[i] = (uint8_t)((mc4[i] + mc5[i]) >> 1);
                    }
                }
            }
            else if (pJPEG->ucSubSample && pJPEG->ucNumComponents == 3) // if color (not CMYK)
            {
                // first chroma
                pJPEG->ucACTable = cACTable1;
                pJPEG->ucDCTable = cDCTable1;
                ...
                if (pJPEG->ucPixelType >= EIGHT_BIT_GRAYSCALE) {
                    ...
                    switch (pJPEG->ucSubSample) {
                        ...
                        case 0x11:
                            ...
                            break;
                        case 0x12:
                        case 0x13: // 三分量都 1x2, 色度已平均进 MCU2/MCU3
